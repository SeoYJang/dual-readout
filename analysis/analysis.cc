...
// 221121 add
  ...
  float fCalibC, fCalibS;
  float fCThres = 32.5;

  std::vector<std::pair<float,float>> fCalibs;
  std::string filename_cal="calib.csv";
  std::ifstream in;
  int i;
  float ceren, scint;

  in.open(filename_cal,std::ios::in);
  while (true) {
    in >> i >> ceren >> scint;
    if (!in.good()) break;
...
// 221121 add
  ...
  //
    ...
    for (auto tower = drEvt.towers.begin(); tower != drEvt.towers.end(); ++tower) {
      ...
      // Tower theta 가져옴.
      ttheta = (int)tower->towerTheta.first;
      if (ttheta < 0) ttheta = -ttheta -1;
      if (ttheta >= (int)fCalibs.size()) continue;

      fCalibC = fCalibs.at(ttheta).first;
      fCalibS = fCalibs.at(ttheta).second;
      rE_C = 0.;
      rE_S = 0.;
      //
      ...
      install/calib.csv file 사용할 것, recoTower->readcsv 참조

      RootIO interface 등등...
      root 명령어들 공부
      TTree* test = new TTree()
      _file0->GetObject("DRsim", test) 등등...
      */ 
